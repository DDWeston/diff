...
TEST(DiffTest, CheckEmptyNoCommonSubsequence) {
  ...
  auto expect = Edits{{DELETE, 0, 5},    // = tokens1[ 0,  5) = all of tokens1
                      {INSERT, 0, 44}};  // = tokens2[ 0, 44) = all of tokens2
  EXPECT_EQ(ToString(actual), ToString(expect));

  // Find the longest common subsequence.
  auto max_elem_iter = std::max_element(
      actual.cbegin(), actual.cend(), [](const Edit &lhs, const Edit &rhs) {
        return (lhs.operation == EQUALS && rhs.operation == EQUALS &&
...
TEST(DiffTest, CheckCharArrayDiffResultsAndLongestCommonSubsequence) {
  ...
  auto expect = Edits{{EQUALS, 0, 4},     // = tokens1[ 0,  4) = "the "
                      {INSERT, 4, 16},    // = tokens2[ 4, 16) = "quick brown "
                      {EQUALS, 4, 15},    // = tokens1[ 4, 15) = "fox jumped "
                      {DELETE, 15, 20},   // = tokens1[15, 20) = "over "
                      {EQUALS, 20, 24},   // = tokens1[20, 24) = "the "
                      {INSERT, 31, 36},   // = tokens2[31, 36) = "lazy "
                      {EQUALS, 24, 27},   // = tokens1[24, 27) = "dog"
                      {DELETE, 27, 28}};  // = tokens1[27, 28) = "."
  EXPECT_EQ(ToString(actual), ToString(expect));
  ...
  ASSERT_NE(0, std::distance(max_elem_iter, actual.cend()));
  ASSERT_EQ(EQUALS, max_elem_iter->operation);
...
TEST(DiffTest, CheckNonConstStringVectorDiffResults) {
  ...
  // EQUALS and DELETE offsets point into tokens1, INSERT into tokens2.
  Edits expect =
      decltype(actual){{EQUALS, 0, 1},   // = tokens1[0, 1) = {"the"}
                       {INSERT, 1, 3},   // = tokens2[1, 3) = {"quick", "brown"}
                       {EQUALS, 1, 3},   // = tokens1[1, 3) = {"fox", "jumped"}
                       {DELETE, 3, 4},   // = tokens1[3, 4) = {"over"}
                       {EQUALS, 4, 5},   // = tokens1[4, 5) = {"the"}
                       {INSERT, 6, 7},   // = tokens2[6, 7) = {"lazy"}
                       {EQUALS, 5, 6},   // = tokens1[5, 6) = {"dog"}
                       {DELETE, 6, 7}};  // = tokens1[6, 7) = {"."}
  EXPECT_EQ(ToString(actual), ToString(expect));
