...
// third_party/diff_match_patch/cpp/diff_match_patch.cpp under the following
...
#ifndef THIRD_PARTY_DIFF_DIFF_H__
#define THIRD_PARTY_DIFF_DIFF_H__
...
enum Operation { EQUALS, DELETE, INSERT };
...
class Diff {
 private:
  ...
  void Generate(int64_t b1, int64_t e1, int64_t b2, int64_t e2,
    ...
    if (prefix_size != 0) {
      InsertEditAt(edits_size, EQUALS, b1, b1 + prefix_size, edits);
    }
    if (suffix_size != 0) {
      AppendEdit(EQUALS, e1 - suffix_size, e1, edits);
    }
  }
    ...
    // Enclose in a scope to save stack space before recursing.
    {
      ...
      if (length1 > length2) {
        ...
        if (offset != e1) {
          ...
          AppendEdit(DELETE, b1, offset, edits);
          AppendEdit(EQUALS, offset, offset_end, edits);
          AppendEdit(DELETE, offset_end, e1, edits);
          return;
        }
        if (length2 == 1) {
          // Single-token span.
        ...
        if (offset != e2) {
          ...
          AppendEdit(INSERT, b2, offset, edits);
          AppendEdit(EQUALS, b1, e1, edits);  // Index into tokens1!
          AppendEdit(INSERT, offset + length1, e2, edits);
          return;
        }
        if (length1 == 1) {
          // Single-token span.
...
#endif  // THIRD_PARTY_DIFF_DIFF_H__
